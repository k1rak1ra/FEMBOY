...
public class set_user_name   extends HANDLER_BASE {

    public set_user_name(final boolean verify_session, final boolean admin, final boolean verify_feature_level) {
    ...
    public void handle(final HttpExchange t) throws IOException {
        ...
        try {
            if (DatabaseAbstractionLayer.set_user_name(p.params.get("target_uid"), p.params.get("name")))
                success(p);
            else
                error(1, p);
